Create Codex pet row `running-right` for `guanguan-nanshanjing-shanhaijing`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Upgrade the existing approved pet atlas to spriteVersionNumber 2. Preserve the exact character identity, style, palette, markings, proportions, and materials from the current atlas; generate only natural looking directions.. Preserve silhouette, face, palette, material, proportions, markings, and props.
